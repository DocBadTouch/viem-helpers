...
const files = glob.sync(path.join(inputDirectory, `**/*.+(${fileTypes.join('|')})`), { absolute: true });
...
files.forEach(file => {
  ...
  const exportAbiString = `export const ${fileName}_ABI = ${JSON.stringify(abi, null, 2)} as const;`;
  ...
  abi.forEach((entry: any) => {
    if (entry.type === 'function') {
      ...
      functionOutputs[entry.name] = entry.outputs.map((output: any) => {if (output.type === 'address') {
          return "`0x${string}`";
  ...
  const functionTypes = Object.keys(functionsObject).map(key => {
    ...
    return `export type ${key}_type = (${inputs.join(', ')}) => Promise<${outputs.join(', ')}>;`;
  });
  ...
  fs.writeFileSync(outputFileName, `${exportAbiString}\n\n${functionsExportString}\n\n${eventsExportString}\n\n${functionTypes.join('\n')}\n\n${eventTypes.join('\n')}`);

  console.log(`Generated TypeScript constants in ${outputFileName}`);
});

// Create the index.ts file to export all constants
